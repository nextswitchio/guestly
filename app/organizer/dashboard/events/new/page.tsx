...
export default function CreateEventPage() {
  ...
  async function publish() {
    ...
    if (res.ok) {
      ...
      router.replace("/organizer/dashboard/events");
    } else {
      ...
      <div className="mx-auto max-w-5xl">
        ...
        {!identityLoading && !isIdentityVerified && (
          <div className="mb-8 flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-5 py-4">
            ...
            <Link href="/organizer/dashboard/settings" className="shrink-0 rounded-lg bg-white px-3 py-2 text-xs font-semibold text-amber-800 ring-1 ring-amber-200 hover:bg-amber-100">
              Verify now
            </Link>
          </div>
        )}
