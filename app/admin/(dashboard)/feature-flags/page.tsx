  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-[var(--foreground-muted)]">Toggle platform features on and off</p>
      </div>

      <Card className="p-6">
        <div className="space-y-4">
          {flags.map((f) => (
            <div key={f.id} className="flex items-center justify-between p-4 border border-[var(--surface-border)] rounded-lg">
              <div>
                <p className="font-medium text-[var(--foreground)]">{f.name}</p>
                <p className="text-sm text-[var(--foreground-muted)]">{f.description}</p>
              </div>
              <div className="flex items-center gap-4">
                <span className="text-sm text-[var(--foreground-muted)]">{f.rollout}% rollout</span>
                <button
                  onClick={() => toggleFlag(f.id)}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${f.enabled ? "bg-lime" : "bg-[var(--surface-secondary)]"}`}
                >
