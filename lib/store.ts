...
const products: Product[] = [
  ...
  { id: "prod-4", eventId: "evt-1", name: "Event Poster", description: "A3 high-quality print of the official event artwork.", price: 8, image: "", category: "Prints", stock: 80, sold: 120, active: true, fulfillmentType: "digital", digitalDownloadUrl: "/downloads/tech-summit-poster.pdf" },
...
/**
 * Initialize some sample disputes for testing
 */
export function initializeSampleDisputes() {
  // Only initialize if no disputes exist
  if (Object.keys(disputes).length > 0) return;
  ...
  const sampleOrders = [
    {
      ...
      items: [{ type: 'General' as TicketType, quantity: 2, price: 5000 }],
      total: 10000,
    ...
    },
    {
      ...
      items: [{ type: 'VIP' as TicketType, quantity: 1, price: 15000 }],
      total: 15000,
      status: 'paid' as const,
  ...
  const sampleDisputes: Dispute[] = [
    {
      ...
      userName: 'John Doe',
      eventId: 'evt_sample_001',
      eventTitle: 'Tech Conference Lagos 2024',
      organizerId: 'org_sample_001',
      reason: 'event_cancelled',
      description: 'The event was cancelled last minute without proper notice. I had already made travel arrangements and booked accommodation. This is very disappointing and I would like a full refund.',
      status: 'open',
      orderAmount: 10000,
    ...
    },
    {
      ...
      userName: 'Jane Smith',
      eventId: 'evt_sample_002',
      eventTitle: 'Music Festival Abuja',
      organizerId: 'org_sample_002',
      reason: 'poor_experience',
      description: 'The sound quality was terrible, the venue was overcrowded, and several advertised artists did not show up. The event was nothing like what was promised.',
      status: 'investigating',
      orderAmount: 15000,
      assignedTo: 'admin_001',
      createdAt: Date.now() - 1 * 24 * 60 * 60 * 1000, // 1 day ago
      updatedAt: Date.now() - 12 * 60 * 60 * 1000, // 12 hours ago
      adminNotes: 'Contacted organizer for response. Waiting for evidence from customer.',
    },
    {
      id: 'dispute_003',
      ...
      eventTitle: 'Business Workshop Accra',
      organizerId: 'org_sample_003',
      reason: 'technical_issues',
      description: 'The virtual event platform kept crashing and I missed most of the sessions. The organizer was not responsive to technical support requests.',
      status: 'resolved',
      orderAmount: 8000,
      refundAmount: 4000,
      resolution: 'Partial refund approved due to technical difficulties. Customer received 50% refund as compensation.',
  ...
  // Add disputes to storage
  sampleDisputes.forEach(dispute => {
    disputes[dispute.id] = dispute;
    disputesByOrder[dispute.orderId] = dispute.id;
    
    if (!disputesByUser[dispute.userId]) {
      disputesByUser[dispute.userId] = [];
    }
    disputesByUser[dispute.userId].push(dispute.id);
  });

  // Create a sample refund for the resolved dispute
  const sampleRefund: RefundProcessing = {
    id: 'refund_001',
    ...
    method: 'wallet',
    status: 'completed',
    reason: 'Dispute resolution: Partial refund approved due to technical difficulties',
    processedBy: 'admin_002',
    createdAt: Date.now() - 3 * 24 * 60 * 60 * 1000,
    completedAt: Date.now() - 3 * 24 * 60 * 60 * 1000,
    transactionId: 'wallet_refund_001',
  };

  refundProcessing[sampleRefund.id] = sampleRefund;
  refundsByDispute['dispute_003'] = sampleRefund.id;

  console.log('Sample disputes and refunds initialized');
}
...
/**
 * Initialize sample featured placements for testing
 */
export function initializeSampleFeaturedPlacements() {
  // Only initialize if no placements exist
  if (Object.keys(featuredPlacements).length > 0) return;

  const now = Date.now();
  const oneDay = 24 * 60 * 60 * 1000;
  const oneWeek = 7 * oneDay;

  const samplePlacements: FeaturedPlacement[] = [
    {
      id: 'featured_001',
      eventId: 'evt_sample_001',
      position: 1,
      startDate: now - oneDay, // Started yesterday
      endDate: now + (5 * oneDay), // Ends in 5 days
      sponsorshipFee: 50000,
      createdBy: 'admin_001',
      createdAt: now - (2 * oneDay),
      updatedAt: now - (2 * oneDay),
      status: 'active',
      notes: 'Premium placement for tech conference',
    },
    {
      id: 'featured_002',
      eventId: 'evt_sample_002',
      position: 2,
      startDate: now + oneDay, // Starts tomorrow
      endDate: now + oneWeek, // Ends in a week
      sponsorshipFee: 30000,
      createdBy: 'admin_001',
      createdAt: now - oneDay,
      updatedAt: now - oneDay,
      status: 'active',
      notes: 'Music festival promotion',
    },
    {
      id: 'featured_003',
      eventId: 'evt_sample_003',
      position: 1,
      startDate: now - oneWeek, // Started a week ago
      endDate: now - oneDay, // Ended yesterday
      sponsorshipFee: 25000,
      createdBy: 'admin_002',
      createdAt: now - (oneWeek + oneDay),
      updatedAt: now,
      status: 'expired',
      notes: 'Business workshop - completed placement',
    },
  ];

  // Add placements to storage
  samplePlacements.forEach(placement => {
    featuredPlacements[placement.id] = placement;
    placementsByEvent[placement.eventId] = placement.id;
    
    if (!placementsByPosition[placement.position]) {
      placementsByPosition[placement.position] = [];
    }
    placementsByPosition[placement.position].push(placement.id);
  });

  console.log('Sample featured placements initialized');
}
 ...
 */
export function initializeSampleFraudAlerts() {
  // Only initialize if no alerts exist
  if (Object.keys(fraudAlerts).length > 0) return;

  const now = Date.now();
  const oneHour = 60 * 60 * 1000;
  const oneDay = 24 * 60 * 60 * 1000;

  const sampleAlerts: Omit<FraudAlert, 'id'>[] = [
    {
      type: 'suspicious_refund_rate',
      severity: 'critical',
      title: 'High refund rate detected for user user_123',
      description: 'User has 8 refunds out of 10 orders (80%) in the past week',
      affectedUsers: ['user_123'],
      affectedOrders: ['ord_001', 'ord_002', 'ord_003'],
      metadata: {
        refundRate: 0.8,
        totalOrders: 10,
        refundedOrders: 8,
    ...
    },
    {
      ...
      severity: 'medium',
      title: 'Potential duplicate accounts detected',
      description: '3 accounts with similar names created within short timeframe',
      affectedUsers: ['user_456', 'user_457', 'user_458'],
      metadata: {
        similarName: 'johnsmith',
        accountCount: 3,
        creationTimes: [now - oneDay, now - oneDay + 300000, now - oneDay + 600000]
      },
      status: 'investigating',
      assignedTo: 'admin_001',
      createdAt: now - oneDay,
    ...
    },
    {
      type: 'velocity_abuse',
      severity: 'high',
      title: 'Velocity abuse detected for user user_789',
      description: 'User made 15 credit transactions in the past hour',
    ...
    },
    {
      ...
      severity: 'medium',
      title: 'High-value transactions detected for user user_101',
      description: 'User made 2 transactions over $1000',
      affectedUsers: ['user_101'],
      metadata: {
        highValueCount: 2,
        amounts: [150000, 200000],
        descriptions: ['Top up', 'Top up']
      },
      status: 'resolved',
      resolution: 'Verified legitimate business transactions',
      createdAt: now - (2 * oneDay),
      updatedAt: now - oneDay,
      resolvedAt: now - oneDay,
    },
  ];
  ...
  sampleAlerts.forEach(alertData => {
    const alert = createFraudAlert(
      ...
      alertData.metadata,
      alertData.affectedEvents,
      alertData.affectedOrders
    );
  ...
  console.log('Sample fraud alerts initialized');
}
...
if (typeof window === 'undefined') {
  ...
  setTimeout(() => {
    initializeSampleDisputes();
    initializeSampleFeaturedPlacements();
    initializeSampleFraudAlerts();
  }, 1000);
}
...
/**
 * Seed demo audit logs (no-op if already seeded)
 */
let auditLogsSeeded = false;
export function seedAuditLogs(): void {
  if (auditLogsSeeded || auditLogs.length > 0) return;
  auditLogsSeeded = true;
  ...
  const targetTypes: AuditLogTargetType[] = ['user', 'event', 'dispute', 'announcement', 'commission'];
