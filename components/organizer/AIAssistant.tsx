...
export default function AIAssistant() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-12rem)] max-h-[800px]">
      <div className="flex-1 flex flex-col overflow-hidden rounded-2xl bg-[#001c24] p-0">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-white/10 p-4">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary-500/20 text-lime">
              <Bot className="h-5 w-5" />
            </div>
            <div>
              <h2 className="text-sm font-bold text-white">AI Planning Assistant</h2>
              <p className="text-[10px] text-white/40 font-medium uppercase tracking-wider">Online • Beta</p>
            </div>
          </div>
        ...
        >
          {messages.map((m, i) => (
            ...
            >
              <div className={`max-w-[80%] rounded-2xl p-4 ${
                ...
                <p className={`mt-2 text-[10px] ${m.role === "user" ? "text-primary-200" : "text-white/40"}`}>
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="bg-white/10 rounded-2xl p-4 rounded-tl-none border border-white/10">
                <div className="flex gap-1">
                  <span className="h-1.5 w-1.5 rounded-full bg-lime animate-bounce" />
                  <span className="h-1.5 w-1.5 rounded-full bg-lime animate-bounce [animation-delay:0.2s]" />
                  <span className="h-1.5 w-1.5 rounded-full bg-lime animate-bounce [animation-delay:0.4s]" />
                </div>
              </div>
            </div>
          )}
        ...
        {/* Quick Actions */}
        <div className="px-6 py-3 bg-white/5 border-t border-white/10 overflow-x-auto">
        ...
        {/* Input Area */}
        <div className="p-4 bg-[#001c24]">
          <div className="relative">
            <input
              type="text"
              placeholder="Ask me anything about your event..."
              className="w-full rounded-xl bg-white/10 border-none px-4 py-3.5 pr-12 text-sm text-white placeholder:text-white/40 focus:ring-2 focus:ring-primary-500 transition-all shadow-inner"
              value={input}
            ...
            <button 
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 flex h-9 items-center gap-1.5 rounded-xl bg-gradient-to-br from-lime to-lime-hover px-3 text-dark font-semibold text-sm shadow-lg shadow-lime/20 hover:shadow-lime/40 hover:scale-105 active:scale-95 disabled:opacity-40 disabled:hover:scale-100 disabled:cursor-not-allowed transition-all duration-200"
            >
              Send
...
function QuickActionButton({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-[11px] font-semibold text-white/70 hover:bg-white/10 hover:text-white hover:border-white/20 transition-all"
    >
      {label}
    </button>
  );
